...
Addindex addindex;
int isExist(int a) {
	for (int i = 0; i < addindex.count; i++) {
		if (a == addindex.addarray[i])
			return 1;
	}
	return 0;
}
int getNextMb_addr(int mb_addr) {
	int seed = 0, newaddr;
	do {
		srand(seed++);
		newaddr = rand() % 99;

	} while (isExist(newaddr));
	addindex.addarray[addindex.count++] = newaddr;
	return newaddr;
}
...
void start_macroblock(Slice *currSlice, Macroblock **currMB, int mb_addr, Boolean mb_field)
{
	...
	int prev_mb;
	//*currMB = &p_Vid->mb_data[getNextMb_addr(mb_addr)];
...
int write_p_slice_MB_layer(Macroblock *currMB, int rdopt, int *coeff_rate)
{
	...
	else if (currMB->mb_type != 0)
	{
		...
		for (int i = 0; i < sizeof(motion) / sizeof(motion[0]); i++)
			for (int j = 0; j < sizeof(motion[0]) / sizeof(motion[0][0]); j++) {
				printf("%d %d x%d y%d\n", i, j, motion[i][j].mv[0].mv_x, motion[i][j].mv[0].mv_y);
				printf("%d %d x%d y%d\n", i, j, motion[i][j].mv[1].mv_x, motion[i][j].mv[1].mv_y);
	}
...
//void  encryptIntraModes(Macroblock *currMB) {
//	//srand(currMB->p_Vid->curr_frm_idx+rand());
//	//int r=rand();
//	////int r = rand() % 7 + 1;
//	//if (currMB->p_Slice->slice_type != 2) {
...
void encryptDCTCoeff(Macroblock*currMB, int block_type, int b8, int b4, int k, int*pLevel) {
	...
	srand(currMB->mb_x + currMB->mb_y + frameindex + block_type + b8 + b4 + k);
	int r = rand();
	if (r & 0x01 == 1) {
		pLevel[k] = -pLevel[k];
	}
