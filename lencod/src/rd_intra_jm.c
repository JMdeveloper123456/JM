...
int getBestPredMode(int intra_pred_mode, int mostProbableMode) {
	if (intra_pred_mode == -1) {
		return mostProbableMode;
	}
	else if (intra_pred_mode < mostProbableMode) {
		return intra_pred_mode;
	}

	return intra_pred_mode + 1;
}
void setipredmodeAfterEncryptAndDecode(Macroblock*currMB,  int all_available, int up_available, 
	int left_available,char** ipredmodeAfterEncryptAndDecode,char*intra_pred_mode, PixelPos left_block, 
	PixelPos top_block,int r,int pic_block_x,int pic_block_y) {
	char upMode = top_block.available ? currMB->p_Vid->ipredmodeAfterEncryptAndDecode[top_block.pos_y][top_block.pos_x] : (char)-1;
	char leftMode = left_block.available ? currMB->p_Vid->ipredmodeAfterEncryptAndDecode[left_block.pos_y][left_block.pos_x] : (char)-1;
	int mostProbableMode = (upMode < 0 || leftMode < 0) ? DC_PRED : upMode < leftMode ? upMode : leftMode;
	int mode = getBestPredMode(*intra_pred_mode, mostProbableMode);
	int available_mode = mode>=0&&mode<=8 && ((all_available) || (mode == DC_PRED) ||
		(up_available && (mode == VERT_PRED || mode == VERT_LEFT_PRED || mode == DIAG_DOWN_LEFT_PRED)) ||
		(left_available && (mode == HOR_PRED || mode == HOR_UP_PRED)));
	if (!available_mode) {
		*intra_pred_mode = *intra_pred_mode^r;
	}
	currMB->p_Vid->ipredmodeAfterEncryptAndDecode[pic_block_y][pic_block_x] = *intra_pred_mode;
}
int  encryptIntraModes_forI4x4MB(Macroblock *currMB, int  b8, int  b4) {
	int frameindex = currMB->p_Slice->frame_num;
	srand(currMB->mb_x + currMB->mb_y + frameindex + b8 + b4);
	int r = rand();
	//int r = rand() % 7 + 1;
	r = r&(0x07);
	currMB->intra_pred_modes[b8 * 4 + b4] = currMB->intra_pred_modes[b8 * 4 + b4] ^ r;
	return r;
}
...
int mode_decision_for_I4x4_blocks_JM_High(Macroblock *currMB, int  b8, int  b4, int  lambda, distblk*  min_cost)
{
	...
	*min_cost = DISTBLK_MAX;
	...
	//encryptIntraModes_forI4x4MB(currMB, b8, b4, all_available, up_available, left_available, left_block, top_block);
	int r=encryptIntraModes_forI4x4MB(currMB, b8, b4);
	setipredmodeAfterEncryptAndDecode(currMB, all_available, up_available, left_available, p_Vid->ipredmodeAfterEncryptAndDecode,
		&currMB->intra_pred_modes[4 * b8 + b4], left_block, top_block, r, pic_block_x, pic_block_y);
	//p_Vid->ipredmodeAfterEncryptAndDecode[pic_block_y][pic_block_x] = currMB->intra_pred_modes[4 * b8 + b4];//用加密之后的预测模式给全局变量赋值
