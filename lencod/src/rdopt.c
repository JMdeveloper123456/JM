	...
	distblk       min_rdcost)
{
	...
	*nonzero = currMB->residual_transform_quant_luma_4x4(currMB, PLANE_Y, block_x, block_y, &dummy, 1);
	...
	{  /*
		for(int j=0;j<2;j++)
		 for (int i = 0; i < 65; i++) {
			printf("%d ", currSlice->cofAC[b8][b4][j][i]);
		}
		int level = 1;
		for (int k = 0; k < 2; k++)
			for (int i = 0; (i <= 16) && level != 0; i++)
		{       
				level = currSlice->cofAC[b8][b4][k][i];
				currSlice->cofAC_write[currMB->mb_x][currMB->mb_y][b8][b4][k][i] =level ;
		}*/
		/*for (int k = 0; k < 2; k++) {
			memcpy(currSlice->cofAC_write[currMB->mb_x][currMB->mb_y][b8][b4][k], currSlice->cofAC[b8][b4][k],65);
		}*/
...
void encryptIntraModes_forI16MB(Macroblock *currMB) {
	int frameindex = currMB->p_Slice->frame_num;
	srand(currMB->mb_x + currMB->mb_y + frameindex);
	int r = rand(), mode, up_avail, left_avail, left_up_avail;
	VideoParameters *p_Vid = currMB->p_Vid;
	InputParameters *p_Inp = currMB->p_Inp;
	PixelPos pix_b;
	PixelPos pix_a;
	...
	{
		up_avail = pix_b.available;
		left_avail = pix_a.available;
		left_up_avail = pix_d.available;
	}
	else
	{
		up_avail = pix_b.available ? p_Vid->intra_block[pix_b.mb_addr] : 0;
		left_avail = pix_a.available ? p_Vid->intra_block[pix_a.mb_addr] : 0;
		left_up_avail = pix_d.available ? p_Vid->intra_block[pix_d.mb_addr] : 0;
	}
	if (currMB->mb_type == I16MB) {
		...
		int notavailablemode = (mode == VERT_PRED_16 && !up_avail) || (mode == HOR_PRED_16 && !left_avail) || (mode == PLANE_16 && (!left_avail || !up_avail || !left_up_avail));
		if (notavailablemode) {
	...
	short    mode)         // <-- mode (0-COPY/DIRECT, 1-16x16, 2-16x8, 3-8x16, 4-8x8(+), 5-Intra4x4, 6-Intra16x16)                            
{
	...
	else if (mode == I16MB)
	{
		...
		encryptIntraModes_forI16MB(currMB);
	}
	else if (mode == I8MB)
	{
